      ...
      (function () {
        ...
        if (isMobile || isSmallScreen) {
          console.log('📱 Mobile detected - redirecting to index.mobile.html');
      ...
      // ⚡ FIX #5: DISABLE CONSOLE PATCHING (causes delays)
      // Only patch in debug mode
      if (DEBUG_MODE) {
        (function patchConsoleLog() {
          const originalLog = console.log.bind(console);
          const allowedPrefixes = ['🔥 ARNOMA', '⚡', '❌', '✅', '🔐', '🚪'];
      ...
      console.log('✅ ARNOMA v2.7.1 - Clean Round Buttons with Data Attributes ✅');
      ...
      async function checkAuthentication() {
        try {
          ...
          if (session && session.user) {
            ...
            debugLog('⚠️ No active session, showing login');
            showLoginForm();
            return false;
          }
        } catch (error) {
      ...
      async function handleLogin(email, password) {
        try {
          ...
          if (data.session) {
            debugLog('✅ Login successful:', data.user.email);
            isAuthenticated = true;
      ...
      window.PopupManager = {
        ...
        const PaymentStore = {
          ...
          fetchAll: async function () {
            ...
            if (window.paymentsCache && window.paymentsCache.length > 0 && cacheAge < 30000) {
              debugLog('⚡ Using cached payments (age:', cacheAge, 'ms)');
              return window.paymentsCache;
            }

            try {
              // ⚡ FIX #6: CAP QUERY - Load only recent payments initially
        ...
        async function addPayment(paymentData) {
          ...
          if (data && data[0]) {
            ...
            if (student && student.email) {
              verboseLog('📧 Auto-sending payment receipt email to:', student.name);
              try {
                ...
                if (emailResult.success) {
                  verboseLog('✅ Payment receipt email sent successfully');
                  // Create notification
                  ...
                  console.error('⚠️ Payment receipt email failed:', emailResult.error);
                }
              } catch (emailError) {
                console.error('⚠️ Payment receipt email failed:', emailError);
        ...
        async function loadStudents() {
          ...
          if (window.studentsCache && window.studentsCache.length > 0 && cacheAge < 30000) {
            debugLog('⚡ Using cached students (age:', cacheAge, 'ms)');
            return window.studentsCache;
          }

          // ⚡ FIX #6: CAP QUERY - Load only active/recent students initially
          const { data, error } = await supabase
            .from('students')
            .select('*')
        ...
        async function saveStudent(studentData) {
          ...
          if (typeof studentData.showInGrid === 'boolean') {
            ...
            console.log('✅ Adding show_in_grid to payload:', payload.show_in_grid);
          } else {
            console.warn('⚠️ showInGrid is not a boolean:', typeof studentData.showInGrid, studentData.showInGrid);
          }
          ...
          console.log('📤 Supabase payload:', isUpdate ? 'UPDATE' : 'INSERT', payload);
          ...
          try {
            ...
            if (mappedRecord && mappedRecord.email && mappedRecord.group) {
              ...
              if (addedGroups.length > 0) {
                verboseLog(`📧 Sending enrollment emails for ${addedGroups.length} new group(s):`, addedGroups);
                ...
                for (const groupName of addedGroups) {
                  try {
                    ...
                    console.error(`⚠️ Enrollment email failed for group ${groupName}:`, emailError);
        ...
        async function loadGroupsFromSupabase() {
          ...
          if (window.groupsCache && window.groupsCache.length > 0 && cacheAge < 30000) {
            debugLog('⚡ Using cached groups (age:', cacheAge, 'ms)');
        ...
        async function saveGroup(groupData) {
          ...
          if (data && data[0] && oldSchedule && oldSchedule !== payload.schedule) {
            ...
            verboseLog(`📧 Schedule changed for ${groupName}: "${oldSchedule}" → "${newSchedule}"`);
            ...
            verboseLog(`📧 Sending schedule change emails to ${groupStudents.length} students`);
            ...
            for (const student of groupStudents) {
              if (student.email) {
                try {
                  ...
                  console.error(`⚠️ Schedule change email failed for ${student.name}:`, emailError);
        ...
        async function processOverpayments() {
          try {
            ...
            debugLog('💰 Processing overpayments for', payments.length, 'payments...');
            ...
            for (const payment of payments) {
              ...
              const student = students.find(s => String(s.id) === String(payment.derivedStudentId));
              if (!student) {
                debugLog(`⚠️ Student ID ${payment.derivedStudentId} not found for payment ${payment.id}`);
                continue;
              }
              ...
              if (false && paymentAmount > studentPrice && studentPrice > 0) {
                ...
                debugLog(
                  `💰 OVERPAYMENT DETECTED: ${student.name} paid $${paymentAmount}, price is $${studentPrice}, excess: $${excess}`
                );
                ...
                try {
                  ...
                  await window.NotificationCenter.add(
                    ...
                    'Credit Added (Auto)',
                    `${student.name} paid $${paymentAmount.toFixed(2)} — $${excess.toFixed(2)} added as credit. Email NOT sent automatically.`,
                    {
                      studentId: student.id,
                      amount: excess,
                  ...
                  debugLog(`✅ Credit added to ${student.name}: $${excess.toFixed(2)} (email not sent)`);
            ...
            if (creditsApplied.length > 0) {
              ...
              showNotification(
                `💰 Applied $${totalCredit.toFixed(2)} in credits to ${creditsApplied.length} student(s). Emails NOT sent - review and send manually if needed.`,
                'success',
                10000 // Show for 10 seconds
              );

              debugLog('✅ Credits applied (emails not sent):', creditsApplied);
              debugLog('📋 Students who received credit:', studentNames);
            } else {
            ...
            showNotification('❌ Error processing overpayments', 'error');
            return [];
          }
        }
        ...
        async function sendCreditAddedEmail(student, creditAmount, newBalance) {
            ...
            <body>
              <div class="container">
                ...
                <div class="content">
                  ...
                  <div class="info-box">
                    <strong style="color: #f59e0b;">💡 How to use your credit:</strong>
          ...
          `;
            ...
            verboseLog(`✅ Credit email sent to ${student.name}`);
            return true;
          } catch (error) {
        ...
        async function sendCreditEmailsToStudents(studentIds) {
          ...
          showNotification(`✅ Sent ${successCount} of ${results.length} credit emails`, 'success');
          return results;
        }
      ...
      `;
        ...
        async function ensureGmailTokenValid() {
          ...
          if (expiry && expiry - now < fiveMinutes) {
            debugLog('🔄 Gmail token expiring soon, refreshing via Supabase...');
            ...
            try {
              ...
              debugLog('✅ Gmail token refreshed successfully');
        ...
        function initiateGmailAuth() {
          ...
          debugLog('⚠️ IMPORTANT: Make sure this redirect URI is authorized in Google Cloud Console:');
          debugLog('🔄 Using authorization code flow with refresh token support');
        ...
        async function handleOAuthCallback() {
          ...
          debugLog('🔍 Checking for OAuth callback - code:', code ? 'Found' : 'None');
          ...
          if (code) {
            try {
              ...
              showNotification('✅ Gmail connected with refresh token! Auto-renewal enabled.', 'success');

              // Clear code from URL
              window.history.replaceState(null, '', window.location.pathname);

              // Auto-fetch payments
              debugLog('📥 Auto-fetching payments after connection...');
              ...
              if (messageResponse.ok) {
                ...
                if (payment) {
                  ...
                  if (processedGmailIds.has(payment.gmailId)) {
                    verboseLog(`⚠️ Skipped duplicate in current batch: ${payment.gmailId}`);
                    continue;
                  }
      ...
      `;
        ...
        async function sendAliasAddedEmail(student, aliasAdded) {
          try {
            verboseLog('[AliasEmail] 📧 Sending alias added email to:', student.name, 'Alias:', aliasAdded);

            const emailFrame = document.querySelector('iframe[src*="email-system-complete.html"]');
            if (!emailFrame) {
              console.error('[AliasEmail] ❌ Email system iframe not found');
            ...
            verboseLog('[AliasEmail] ✅ Message sent to email system iframe');
            ...
            return new Promise((resolve, reject) => {
              ...
              const handler = event => {
                if (event.data.action === 'aliasEmailSent') {
                  ...
                  verboseLog('[AliasEmail] ✅ Received confirmation from email system');
                  resolve({ success: true, data: event.data });
                } else if (event.data.action === 'aliasEmailError') {
                  clearTimeout(timeout);
                  window.removeEventListener('message', handler);
                  console.error('[AliasEmail] ❌ Email error:', event.data.error);
            ...
            console.error('[AliasEmail] ❌ Error sending alias email:', error);
            return { success: false, error: error.message };
          }
        }
        // ============================================================
        ...
        async function confirmEmailSend(emailType, recipientName, recipientEmail, preview) {
          ...
          `;
            ...
            cancelBtn.onclick = () => {
              modal.remove();
              verboseLog('[EmailConfirmation] ❌ User cancelled email send');
              resolve(false);
            };

            sendBtn.onclick = () => {
              modal.remove();
              verboseLog('[EmailConfirmation] ✅ User confirmed email send');
              resolve(true);
            };
          });
        }
        ...
        async function sendCreditAddedEmail(student, creditAmount, newBalance, reason = 'Extra payment received') {
          try {
            verboseLog('[CreditEmail] 📧 Sending credit added email to:', student.name, 'Amount:', creditAmount);

            const emailFrame = document.querySelector('iframe[src*="email-system-complete.html"]');
            if (!emailFrame) {
              console.error('[CreditEmail] ❌ Email system iframe not found');
              return { success: false, error: 'Email system not loaded' };
            }
            ...
            verboseLog('[CreditEmail] ✅ Message sent to email system iframe');
            ...
            return new Promise((resolve, reject) => {
              ...
              const handler = event => {
                if (event.data.action === 'creditAddedEmailSent') {
                  clearTimeout(timeout);
                  window.removeEventListener('message', handler);
                  verboseLog('[CreditEmail] ✅ Received confirmation from email system');
                  resolve({ success: true, data: event.data });
                } else if (event.data.action === 'creditAddedEmailError') {
                  clearTimeout(timeout);
                  window.removeEventListener('message', handler);
                  console.error('[CreditEmail] ❌ Email error:', event.data.error);
                  resolve({ success: false, error: event.data.error });
                }
              };
            ...
            console.error('[CreditEmail] ❌ Error sending credit added email:', error);
            return { success: false, error: error.message };
          }
        }
        ...
        async function sendCreditAppliedEmail(student, classDate, amountDeducted, newBalance) {
          try {
            verboseLog('[CreditEmail] 📧 Sending credit applied email to:', student.name, 'Class:', classDate);
            ...
            console.error('[CreditEmail] ❌ Error sending credit applied email:', error);
            return { success: false, error: error.message };
          }
        }
        ...
        async function sendCreditManualEditEmail(student, oldBalance, newBalance) {
          try {
            verboseLog('[CreditEmail] 📧 Sending manual credit edit email to:', student.name);
            ...
            console.error('[CreditEmail] ❌ Error sending manual credit edit email:', error);
            return { success: false, error: error.message };
          }
        }
        // ============================================================
        ...
        async function confirmLinkStudent(studentId) {
          ...
          if (!paymentDataSnapshot && savedPaymentDataForLinking) {
            debugLog('⚠️ currentPaymentPopupData was null, using savedPaymentDataForLinking');
          ...
          try {
            ...
            if (payerName && payerName.trim()) {
              ...
              debugLog('🔍 Normalized payer:', normalizedPayerName);
              ...
              if (normalizedPayerName !== normalizedStudentName && !aliasExists) {
                ...
                if (!savedAliasUpdate) {
                  ...
                  debugLog('✅ Alias added successfully:', addedAlias);
                }
              } else {
                debugLog('⏭️ Skipped adding alias (already exists or same as student name)');
              }
            } else {
            }
            ...
            // ============================================================
            // CHECK FOR EXTRA PAYMENT → CREATE CREDIT
            // ============================================================
            ...
            verboseLog('[Credit] 💰 Payment amount:', paymentAmount, 'Price per class:', pricePerClass);
            ...
            verboseLog('[Credit] 📊 Unpaid classes:', unpaidClassCount);
            ...
            if (false && paymentAmount > pricePerClass && unpaidClassCount === 0) {
              ...
              if (savedStudent) {
                ...
                if (student.email) {
                  verboseLog('[Credit] 📧 Sending credit added email...');
                  ...
                  if (emailResult.success) {
                    showNotification(
                      `💳 $${creditAddedAmount.toFixed(2)} added to credit for ${student.name}`,
                      'success'
                    );
                    showNotification(`📧 Credit notification sent to ${student.email}`, 'success');
                    verboseLog('[Credit] ✅ Credit email sent successfully');
                  } else {
                    showNotification(
                      `💳 $${creditAddedAmount.toFixed(2)} added to credit for ${student.name}`,
                      'success'
                    );
                    showNotification(`⚠️ Credit added but email failed: ${emailResult.error}`, 'warning');
                    console.error('[Credit] ❌ Email failed:', emailResult.error);
                  }
                } else {
                  showNotification(
                    `💳 $${creditAddedAmount.toFixed(2)} added to credit for ${student.name}`,
                ...
                showNotification('❌ Failed to save credit balance', 'error');
            ...
            if (aliasWasAdded && addedAlias && student.email) {
              showNotification(`✅ Alias added for ${student.name} (payer: ${addedAlias})`, 'success');

              // Send email notification to student
              verboseLog('[AliasLink] 📧 Sending alias added email...');
              const emailResult = await sendAliasAddedEmail(student, addedAlias);

              if (emailResult.success) {
                showNotification(`📧 Email sent to ${student.email}`, 'success');
                verboseLog('[AliasLink] ✅ Email sent successfully');
              } else {
                showNotification(`⚠️ Alias added but email failed: ${emailResult.error}`, 'warning');
                console.error('[AliasLink] ❌ Email failed:', emailResult.error);
              }
            } else {
            ...
            // ============================================================
            if (student.email) {
              verboseLog('[PaymentReceipt] 📧 Sending payment receipt email...');
              const paymentAmount = parseFloat(payment.amount) || 0;
              ...
              if (emailResult.success) {
                showNotification(`📧 Payment receipt sent to ${student.email}`, 'success');
                verboseLog('[PaymentReceipt] ✅ Email sent successfully');
              } else {
                showNotification(`⚠️ Payment linked but receipt email failed: ${emailResult.error}`, 'warning');
                console.error('[PaymentReceipt] ❌ Email failed:', emailResult.error);
              }
            }
            // ============================================================
            ...
            debugLog('🧹 Cleared savedPaymentDataForLinking after successful link');
        ...
        async function performFullSync() {
          ...
          if (!fromDateStr || !toDateStr) {
            ...
            debugLog('📅 Applied default dates - From:', fromDateStr, 'To:', toDateStr);
          }
        ...
        function startAutoRefresh() {
          ...
          autoRefreshInterval = setInterval(() => {
            ...
            if (!getGmailConnection()) {
              debugLog('⏸️ Skipping auto-refresh - Gmail not connected');
              return;
            }
            debugLog('🔄 Auto-refreshing payments...');
            refreshPayments();
        ...
        async function loadUserPreferences() {
          try {
            ...
            if (!user) {
              console.warn('⚠️ No authenticated user - cannot load preferences');
              return null;
            }
            ...
            debugLog('✅ Loaded user preferences from cloud:', data);
            return data;
          } catch (error) {
        ...
        async function saveTimezonePreference(winter, summer) {
          try {
            ...
            if (!user) {
              console.warn('⚠️ No authenticated user - cannot save preferences');
              return false;
            }
            ...
            userPreferencesCache = data;
            debugLog('✅ Timezone preference saved to cloud:', { winter, summer });
        ...
        async function setLAOffset(hours, isEnabled, { silent = false } = {}) {
          ...
          if (hours === 12 && toggle12) {
            ...
            if (!silent) {
              showNotification(
                isEnabled
                  ? '❄️ Winter offset enabled: –12 hours (synced to cloud)'
                  : '✅ Winter offset disabled (synced to cloud)',
                'success'
              );
            }
            ...
            if (!silent) {
              showNotification(
                isEnabled
                  ? '☀️ Summer offset enabled: –11 hours (synced to cloud)'
                  : '✅ Summer offset disabled (synced to cloud)',
                'success'
              );
            }
          }
        ...
        function updateToggleStyle(toggleElement, isActive) {
          if (!toggleElement) {
            console.warn('⚠️ updateToggleStyle: toggleElement is null');
            return;
          }
          ...
          verboseLog('🎨 Updating toggle style:', {
          ...
          if (slider) {
            ...
            console.warn('⚠️ Slider not found for', toggleElement.id);
          }

          if (knob) {
            knob.style.transform = isActive ? 'translateX(20px)' : 'translateX(0)';
          } else {
            console.warn('⚠️ Knob not found for', toggleElement.id);
          }
        }
        ...
        async function initializeLAOffsetToggles() {
          try {
            ...
            verboseLog('🔧 Initializing toggles:', { enable12, enable11, settings });
            ...
            verboseLog('✅ LA offset toggles initialized:', { winter: enable12, summer: enable11 });
          } catch (error) {
            console.error('❌ Failed to initialize LA offset toggles:', error);
          }
        }
        ...
        function subscribeToPreferenceChanges() {
          supabase
            ...
            .on(
              ...
              async payload => {
                debugLog('🔔 Real-time preference update received:', payload);
                ...
                if (payload.eventType === 'UPDATE' || payload.eventType === 'INSERT') {
                  ...
                  if (toggle12 && toggle11) {
                    ...
                    if (winter) {
                      showNotification('🔄 Synced from another device: Winter offset (–12h) enabled', 'info');
                    } else if (summer) {
                      showNotification('🔄 Synced from another device: Summer offset (–11h) enabled', 'info');
                    } else {
                      showNotification('🔄 Synced from another device: All offsets disabled', 'info');
                    }
                  }
                }
              }
            ...
            .subscribe(status => {
              if (status === 'SUBSCRIBED') {
                debugLog('✅ Subscribed to real-time preference sync');
              } else if (status === 'CHANNEL_ERROR') {
                console.error('❌ Real-time preference sync error');
              }
            });
        }
        ...
        function openStudentManager() {
          ...
          // ⚡ PERFORMANCE: Lazy load student manager on first open
        ...
        function renderStudents() {
          ...
          const visibleUniqueGroups = uniqueGroups.filter(groupName => 
            activeGroups.some(g => g.name === groupName)
          );
        ...
        <div class="student-edit">
          ...
          <div class="inline-form-group">
            <label style="color: #eab308; font-weight: 700;">💰 Credit Balance</label>
      ...
      `;
        ...
        async function changeStudentGroup(studentId, newGroup) {
          ...
          if (window.NotificationCenter) {
            await window.NotificationCenter.add(
              ...
              `Group Changed: ${oldGroup} → ${newGroup}`,
        ...
        async function saveInlineEdit(studentId) {
          debugLog('🚀 saveInlineEdit() called for studentId:', studentId);
          ...
          if (oldAliases !== newAliases) {
            ...
            verboseLog('🏷️ Aliases changed:', `"${oldAliases}"`, '→', `"${newAliases}"`);
          }

          console.log('📝 Total changes detected:', changes.length, changes);
          ...
          verboseLog('💾 Student saved successfully (inline):', savedRecord.name);
          debugLog('📧 Student email:', savedRecord.email);
          debugLog('🔄 Changes to notify:', changes);
          ...
          if (balanceChanged && savedRecord.email && savedRecord.email.trim() !== '') {
            verboseLog('💳 Credit balance changed - asking user about email notification');
            ...
            if (sendEmail) {
              verboseLog('💳 User confirmed - sending credit manual edit email');
              ...
              if (emailResult.success) {
                ...
                showNotificationSimple(`📧 Email sent to ${savedRecord.email}`, 'success');
                verboseLog('[Credit] ✅ Manual edit email sent successfully');
              } else {
                showNotificationSimple(`💳 Credit balance updated for ${savedRecord.name}`, 'success');
                showNotificationSimple(`⚠️ Balance updated but email failed: ${emailResult.error}`, 'warning');
                console.error('[Credit] ❌ Email failed:', emailResult.error);
              }
            } else {
              verboseLog('💳 User declined - credit updated without email');
              showNotificationSimple(`💳 Credit balance updated for ${savedRecord.name}`, 'success');
            }
          ...
          if (changes.length > 0 && savedRecord.email && savedRecord.email.trim() !== '') {
            debugLog('✅ Sending profile update email for', changes.length, 'changes');
            sendProfileUpdateEmail(savedRecord, changes);
          } else {
            verboseLog('⏭️ Skipping profile update email');
        ...
        async function cycleStatus(studentId) {
          ...
          if (window.NotificationCenter) {
            ...
            await window.NotificationCenter.add(
              ...
              `Status Changed: ${statusLabels[oldStatus]} → ${statusLabels[newStatus]}`,
        ...
        async function toggleVisibility(studentId, visible) {
          console.log('🔄 toggleVisibility called:', { studentId, visible });
          ...
          console.log('📝 Before update:', { id: student.id, name: student.name, showInGrid: student.showInGrid });
          
          student.showInGrid = visible;

          console.log('📝 After update:', { id: student.id, name: student.name, showInGrid: student.showInGrid });

          // Save to Supabase
          const result = await saveStudent(student);
          
          if (!result) {
            console.error('❌ Failed to save student visibility');
        ...
        async function saveStudentFromModal() {
          debugLog('🚀 saveStudentFromModal() called');
          ...
          if (editingStudentId) {
            ...
            if (oldStudent) {
              debugLog('🔍 Checking for profile changes...');
              ...
              if (oldStudent.payPerClass !== payPerClass) {
                ...
                verboseLog('💵 Amount per class changed:', oldStudent.payPerClass, '→', payPerClass);
              }
              if (oldStudent.balance !== balance) {
                changes.push({ field: 'Balance', old: `$${oldStudent.balance || 0}`, new: `$${balance}` });
                debugLog('💰 Balance changed:', oldStudent.balance, '→', balance);
              }
          ...
          verboseLog('💾 Student saved successfully:', savedRecord.name);
          debugLog('📧 Student email:', savedRecord.email);
          debugLog('🔄 Changes to notify:', changes);
          ...
          if (nowHasEmail && (isNewStudent || hadNoEmail)) {
            verboseLog('📨 Sending welcome email...');
            sendWelcomeEmail(savedRecord);
          }

          // Send profile update email if there are changes and student has email
          debugLog('🔍 Checking if should send profile update email...');
        ...
        async function sendWelcomeEmail(student) {
          if (!student.email) {
            debugLog('⚠️ Cannot send welcome email - student has no email address');
            return;
          }
          ...
          try {
            ...
            debugLog('📧 Welcome email sent to:', student.name, student.email);
            showNotificationSimple('📧 Welcome email sent to ' + student.name, 'success');
          } catch (error) {
        ...
        async function sendProfileUpdateEmail(student, changes) {
          ...
          try {
            const emailFrame = document.querySelector('iframe[src*="email-system-complete.html"]');
            if (!emailFrame) {
              console.warn('⚠️ Email iframe not found');
              return;
            }
            ...
            for (const recipient of emailsToNotify) {
              ...
              verboseLog(`📧 Profile update email sent to (${recipient.type}):`, student.name, recipient.email);
            }

            verboseLog('📝 Changes notified:', changes.map(c => c.field).join(', '));

            const notificationMessage = emailChange
              ? `📧 Profile update emails sent to old and new addresses`
              : `📧 Profile update email sent to ${student.name}`;
        ...
        async function sendStatusChangeToGraduatedEmail(student, groupName) {
          try {
            ...
            const confirmed = await confirmEmailSend(
              'Status Change: Active → Graduated',
              student.name,
              student.email,
        ...
        async function deleteStudent(studentId) {
          verboseLog('🗑️ deleteStudent called with ID:', studentId, 'Type:', typeof studentId);
          ...
          debugLog('📋 Students in cache:', students.length);

          if (!students || students.length === 0) {
            verboseLog('📥 Cache empty, loading students from Supabase...');
          ...
          verboseLog('👤 Attempting to delete student:', student.name);
          ...
          debugLog('✅ User confirmation:', confirmed);
        ...
        async function toggleGroupVisibility(groupName, visible) {
          ...
          showToast(visible ? '✅ Group visible' : '👁️‍🗨️ Group hidden');
        }
      ...
      `;
        ...
        async function saveScheduleEdit(groupName) {
          ...
          debugLog('📅 Schedule Update Check:', {
            group: groupName,
            oldSchedule: oldSchedule,
            newSchedule: newSchedule,
          ...
          if (scheduleChanged) {
            debugLog('📧 Schedule changed, sending update emails to all students in', groupName);
            ...
            if (studentsInGroup.length > 0) {
              ...
              if (emailFrame && emailFrame.contentWindow) {
                ...
                debugLog('✅ Schedule update email request sent to email system');
                showNotificationSimple(
                  `📧 Schedule update emails sent to ${studentsInGroup.length} student${studentsInGroup.length !== 1 ? 's' : ''}`,
                  'success'
                );
              } else {
                console.warn('⚠️ Email iframe not found, cannot send schedule update emails');
              }
            } else {
              verboseLog('ℹ️ No students in this group to notify');
            }
          }
        ...
        // ⚡ FIX #9: PREVENT DUPLICATE INITIALIZATION
        let _appInitialized = false;
        let isInitialized = false;
        let initializePromise = null;

        async function initialize() {
          // ⚡ FIX #9: Guard against duplicate initialization calls
        ...
        async function _initializeInternal() {
          debugLog('🚀 Initializing ARNOMA app with Supabase...');
          ...
          // ⚡ FIX #6: CAP SUPABASE QUERIES for mobile performance
          try {
        ...
        // ⚡ FIX #2 & #3: PREVENT EMAIL DATA SPAM with mutex
        ...
        function initializeEmailSystemIframe() {
          // ⚡ FIX #3: Check if iframe already exists
          const existingIframe = document.querySelector('iframe[src*="email-system-complete.html"]');
          if (existingIframe) {
            debugLog('⚠️ Email system iframe already exists, skipping initialization');
            return;
          }
          ...
          debugLog('✅ Hidden email system iframe initialized');

          // ⚡ FIX #3: Listen for data requests from automation engine (ONLY ONCE)
          if (!_emailMessageHandlerAdded) {
            ...
            window.addEventListener('message', async function (event) {
              ...
              if (event.data.action === 'openEmailPreview') {
                verboseLog('[EmailPreview] 📧 Received email preview request from iframe');
              ...
              if (event.data.action === 'addNotification') {
                console.log(
                  '[NotificationCenter] 📬 Received notification request from iframe:',
                ...
                if (window.NotificationCenter && event.data.notification) {
                  ...
                  debugLog('[NotificationCenter] ✅ Notification added from iframe');
                }
              }
            });

            debugLog('✅ Email system message listener attached');
          }
        }

        // Email Preview Modal Functions
        function openEmailPreviewModal(htmlContent, subject) {
          verboseLog('[EmailPreview] 🔵 Opening email preview modal in parent window');
          const modal = document.getElementById('emailPreviewModal');
          const iframe = document.getElementById('emailPreviewFrame');

          if (!modal || !iframe) {
            console.error('[EmailPreview] ❌ Modal or iframe not found!');
            return;
          }
          ...
          modal.style.display = 'flex';
          verboseLog('[EmailPreview] ✅ Modal opened successfully');
        }

        function closeEmailPreviewModal() {
          verboseLog('[EmailPreview] 🔴 Closing email preview modal');
          const modal = document.getElementById('emailPreviewModal');
          if (modal) {
            modal.style.display = 'none';
          }
        ...
        // ⚡ PERFORMANCE: Add guard to prevent message spam
        ...
        function sendGroupsDataToEmailSystem(reason = 'manual') {
          ...
          const emailFrame = document.querySelector('iframe[src*="email-system-complete.html"]');
          if (!emailFrame || !emailFrame.contentWindow) {
            verboseLog('⚠️ Email iframe not found or not ready, skipping automation data send');
          ...
          debugLog(`📤 Sent automation data (${reason}): ${groups.length} groups, ${students.length} students`);
        ...
        function renderQuickView() {
          ...
          try {
            ...
            if (!Array.isArray(groupsData) || groupsData.length === 0) {
              contentEl.innerHTML =
                '<div style="text-align: center; padding: 60px; color: rgba(255,255,255,0.5); font-size: 16px;"><div style="font-size: 64px; margin-bottom: 16px;">📚</div><p>No active groups found.<br>Add groups in Group Manager to see schedules here.</p></div>';
              return;
            }
      ...
      */
                ...
                groups.forEach(group => {
                  ...
                  sessions.forEach(session => {
                    ...
                    if (totalDiffSeconds < -7200) {
                      debugLog(`[ClassCountdownTimer] ❌ Skipping ${group.name} - more than 2h past`);
                      return;
                    }
        ...
        // ============================================================================
        // SKIP CLASS MANAGER - Smart Calendar + Timer Integration ☁️ CLOUD SYNC
        // ============================================================================
        /**
         ...
         * - Undo functionality
         * - ☁️ CLOUD SYNC - Works across all devices via Supabase
         */
        window.SkipClassManager = (function () {
          ...
          async function loadSkippedClasses() {
            try {
              ...
              if (error) {
                console.warn('[SkipClassManager] ⚠️ Supabase load error (using localStorage):', error.message);
              ...
              debugLog('✅ [SkipClassManager] Loaded from localStorage + Supabase');
              return merged;
            } catch (error) {
              console.error('[SkipClassManager] ❌ Exception loading skipped classes:', error);
          ...
          function loadSkippedClassesFromLocalStorage() {
            try {
              ...
              debugLog('📦 [SkipClassManager] Loaded from localStorage (fallback)');
              return data ? JSON.parse(data) : {};
            } catch (error) {
              console.error('[SkipClassManager] ❌ Error loading from localStorage:', error);
              return {};
            }
          }
          ...
          async function saveSkippedClasses() {
            try {
              ...
              if (deleteError) {
                console.error('[SkipClassManager] ❌ Error clearing Supabase:', deleteError);
              }
              ...
              if (records.length > 0) {
                ...
                if (insertError) {
                  console.error('[SkipClassManager] ❌ Error saving to Supabase:', insertError);
                  verboseLog('[SkipClassManager] 📦 Using localStorage as primary storage');
                  saveSkippedClassesToLocalStorage(); // Fallback
                  return;
                }

                debugLog('✅ [SkipClassManager] Saved', records.length, 'skipped classes to Supabase');
              } else {
                debugLog('✅ [SkipClassManager] Cleared all skipped classes from Supabase');
              }

              // Also save to localStorage as backup
              saveSkippedClassesToLocalStorage();
            } catch (error) {
              console.error('[SkipClassManager] ❌ Exception saving skipped classes:', error);
              saveSkippedClassesToLocalStorage(); // Fallback
            }
          }
          ...
          function saveSkippedClassesToLocalStorage() {
            try {
              ...
              debugLog('📦 [SkipClassManager] Saved to localStorage (backup)');
            } catch (error) {
              console.error('[SkipClassManager] ❌ Error saving to localStorage:', error);
            }
          }
          ...
          function cancelClass(groupName, dateStr, note = null) {
            ...
            console.log('🚫 [SkipClassManager] Class canceled:', groupName, dateStr, note || '(no reason)');
          ...
          async function forwardPaymentsForCanceledClass(groupName, canceledDateStr) {
            try {
              console.log('💸 [PaymentForwarding] Checking for payments on canceled date:', groupName, canceledDateStr);
              ...
              if (!groupStudents || groupStudents.length === 0) {
                console.log('💸 [PaymentForwarding] No students found in group:', groupName);
                return;
              }
              ...
              if (error) {
                console.error('💸 [PaymentForwarding] Error loading payments:', error);
                return;
              }

              if (!payments || payments.length === 0) {
                console.log('💸 [PaymentForwarding] No payments found on canceled date');
                return;
              }

              console.log(`💸 [PaymentForwarding] Found ${payments.length} payment(s) to forward`);
              ...
              for (const payment of payments) {
                ...
                if (!nextClassDate) {
                  console.warn('💸 [PaymentForwarding] No future class found for student:', student.name);
                  continue;
                }
                ...
                if (updateError) {
                  console.error('💸 [PaymentForwarding] Error updating payment:', updateError);
                } else {
                  console.log(
                    `💸 [PaymentForwarding] ✅ Forwarded payment for ${student.name}: ${canceledDateStr} → ${nextClassDate}`
                  );
                }
              }
              ...
              console.error('💸 [PaymentForwarding] Exception:', error);
            }
          }
          ...
          function findNextActiveClassDate(groupName, fromDateStr) {
            ...
            if (!group || !group.schedule) {
              console.warn('💸 [PaymentForwarding] Group not found or has no schedule:', groupName);
              return null;
            }
            ...
            // Get class days from schedule (e.g., "Mon, Wed 6:00 PM" → [1, 3])
            const classDays = [];
            ...
            if (classDays.length === 0) {
              console.warn('💸 [PaymentForwarding] Could not parse schedule:', schedule);
              return null;
            }
          ...
          function confirmCancelClass() {
            ...
            // Show success notification
            showNotification(`🚫 Entire class canceled: ${pendingSkip.groupName} on ${pendingSkip.dayName}`, 'success');
          }
        ...
        `;
          ...
          async function init() {
            if (isInitialized) {
              debugLog('⚠️ [SkipClassManager] Already initialized, skipping');
              return;
            }

            // debugLog('🚀 [SkipClassManager] Initializing with Supabase cloud sync...');

            // Load skipped classes from Supabase
            skippedClasses = await loadSkippedClasses();
            isInitialized = true;

            // debugLog('✅ [SkipClassManager] Initialized with', Object.keys(skippedClasses).length, 'groups');
          ...
          return {
            ...
            reloadFromSupabase: async () => {
              debugLog('🔄 [SkipClassManager] Reloading from Supabase...');
              skippedClasses = await loadSkippedClasses();
              debugLog('✅ [SkipClassManager] Reloaded', Object.keys(skippedClasses).length, 'groups');
        ...
        window.AbsentManager = (function () {
          ...
          async function loadAbsences() {
            try {
              ...
              // debugLog('✅ Loaded absences from Supabase:', data);
          ...
          function loadAbsencesFromLocalStorage() {
            ...
            debugLog('📦 Loaded absences from localStorage:', absences);
          }
          ...
          async function init() {
            ...
            // debugLog('🚀 Initializing AbsentManager...');
            await loadAbsences();
            isInitialized = true;
          }
          ...
          return {
            ...
            async markAbsent(studentId, dateStr) {
              ...
              if (student && student.email) {
                debugLog('📧 Sending absence notification email to:', student.name);
                try {
                  ...
                  console.error('⚠️ Absence notification email failed:', emailError);
        ...
        window.CreditPaymentManager = (function () {
          ...
          async function loadCreditPayments() {
            try {
              ...
              // debugLog('✅ Loaded credit payments from Supabase:', data);
              ...
              // console.log('📊 Credit payments loaded:', creditPayments);
            } catch (err) {
          ...
          function loadCreditPaymentsFromLocalStorage() {
            ...
            debugLog('📦 Loaded credit payments from localStorage:', creditPayments);
          }
        ...
        function updateStudentBreakdown() {
          ...
          // Show ACTIVE and PAUSED students only — exclude GRADUATED entirely
          ...
          <div style="padding: 16px; background: ${cardBg}; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; transition: all 0.2s; opacity: ${cardOpacity}; filter: ${cardFilter};" onmouseover="this.style.background='${cardBgHover}'" onmouseout="this.style.background='${cardBg}'">
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
              <div style="flex: 1;">
                ...
                <div style="font-size: 12px; color: #94a3b8;">${isPaused ? '⏸ ' : ''}Group ${escapeHtml(data.group)} • ${data.weeklyClasses} classes/week • ${formatCurrency(data.pricePerClass, '$')}/class</div>
              </div>
            </div>
        ...
        async function cycleStatusInBreakdown(studentId) {
          ...
          showNotificationSimple(`✅ ${student.name} status updated to ${newStatus}`, 'success');
        }
        ...
        function openNotificationCenter() {
          ...
          debugLog('[NotificationCenter] 🔔 Opening notification center...');

          // Reload notifications from Supabase to get latest
          if (window.NotificationCenter) {
            window.NotificationCenter.initialize().then(() => {
              debugLog('[NotificationCenter] 🔄 Reloaded notifications from Supabase');
              renderNotifications();
            });
          } else {
            renderNotifications();
        ...
        async function clearAllNotifications() {
          debugLog('[NotificationCenter] 🗑️ Clearing all notifications...');

          if (!window.NotificationCenter) {
            console.error('[NotificationCenter] ❌ NotificationCenter not initialized');
            return;
          }

          // Clear all notifications
          const allNotifications = window.NotificationCenter._notifications || [];
          ...
          debugLog('[NotificationCenter] ✅ All notifications cleared');

          // Reload the notifications view
          await window.NotificationCenter.initialize();
          renderNotifications();

          showNotificationSimple('🗑️ All notifications cleared', 'success');
        }
      ...
      `;
          ...
          if (notification.studentName || notification.groupName) {
            ...
            const parts = [];
            if (notification.studentName) parts.push(`👤 ${notification.studentName}`);
            if (notification.groupName) parts.push(`📚 ${notification.groupName}`);
        ...
        function getNotificationStyle(type) {
          const styles = {
            ...
            update: { icon: '📝', color: '#8ab4ff' },
            reminder: { icon: '🔔', color: '#f59e0b' },
            system: { icon: '⚙️', color: '#94a3b8' },
            status_change: { icon: '🔄', color: '#a78bfa' },
            group_change: { icon: '📚', color: '#ec4899' },
            schedule_update: { icon: '📅', color: '#06b6d4' },
        ...
        `;
          ...
          <div>
            <div style="font-size: 18px; font-weight: 700; color: #fff; margin-bottom: 4px;">
              📧 ${metadata.emailSubject || 'Email Preview'}
            </div>
        ...
        function renderCalendar() {
          ...
          if (!window._calendarInitializedDispatched) {
            ...
            debugLog('[Calendar] 📊 Dispatching calendar:initialized event for automation engine');
        ...
        <div style="display: flex; align-items: center; justify-content: center; gap: 12px;">
          <span>${activeStudentCount} active student${activeStudentCount !== 1 ? 's' : ''} • ${totalClasses} scheduled class${totalClasses !== 1 ? 'es' : ''}</span>
          <span style="color: rgba(255,255,255,0.3);">|</span>
          <span style="color: #22c55e; font-weight: 600;">💰 ${formatCurrency(monthPaid, '$')} paid</span>
          <span style="color: rgba(255,255,255,0.3);">|</span>
          <span style="color: #ef4444; font-weight: 600;">⚠️ ${formatCurrency(monthUnpaid, '$')} pending</span>
        </div>
      `;
        ...
        function renderCalendarGrid(calendarData) {
          ...
          calendarData.days.forEach(dayData => {
            ...
            const hasSkippedClass =
              ...
              dayData.studentsWithClass.some(classInfo => {
                ...
                return (
                  groupName && window.SkipClassManager && window.SkipClassManager.isClassSkipped(groupName, dateStr)
                );
              });

            // Determine cell styling - don't gray out entire cell, only individual dots
            const cellBgColor = isToday ? 'rgba(138,180,255,0.08)' : 'rgba(255,255,255,0.02)';
            const cellBorderColor = isToday ? 'rgba(138,180,255,0.3)' : 'rgba(255,255,255,0.05)';
          ...
          <div onclick="openDayDetails('${dateStr}', ${JSON.stringify(dayData).replace(/"/g, '&quot;')})"
            ...
            <div style="font-size: 14px; font-weight: 700; color: ${isToday ? '#8ab4ff' : 'white'}; margin-bottom: 8px;">
              ${dayData.day}
              ${isToday ? '<span style="font-size: 9px; color: #8ab4ff; margin-left: 4px;">TODAY</span>' : ''}
              ${hasSkippedClass ? '<span style="font-size: 9px; color: rgba(150,150,150,0.7); margin-left: 4px;">⚫ CANCELED</span>' : ''}
            </div>
        ...
        window.toggleAutoReminder = async function (studentId, buttonElement) {
          if (!window.PaymentReminderManager) {
            showNotification('❌ Payment Reminder Manager not available', 'error');
            return;
          }
          ...
          if (success) {
            ...
            buttonElement.textContent = isPaused ? '▶' : '⏸';
        ...
        window.sendReminderNow = async function (studentId, buttonElement, clickedDateStr) {
          debugLog('🚀 Manual send triggered for student:', studentId, 'Clicked date:', clickedDateStr);
          ...
          try {
            ...
            if (!students || students.length === 0) {
              console.error('❌ No students in cache');
              showNotificationSimple('❌ Error: Student data not loaded', 'error');
              return;
            }
            ...
            if (!student) {
              ...
              console.log('Available student IDs:', students.map(s => s.id).slice(0, 10));
              showNotificationSimple('❌ Student not found', 'error');
              return;
            }
            ...
            if (!student.email || student.email.trim() === '') {
              ...
              showNotificationSimple('❌ ' + student.name + ' has no email address', 'error');
              return;
            }

            // Find ALL unpaid classes from calendar data (not just clicked date)
            debugLog('🔍 Searching for ALL unpaid classes in calendar...');
            const currentMonth =
            ...
            // Get student's enrollment date for filtering
            const studentCreatedAt = student.created_at ? new Date(student.created_at) : null;
            if (studentCreatedAt) {
              studentCreatedAt.setHours(0, 0, 0, 0); // Normalize to start of day
            }

            // Search through all days in current month for unpaid classes
            currentMonth.days.forEach(dayData => {
              const dayDate = new Date(dayData.date);
              dayDate.setHours(0, 0, 0, 0); // Normalize to start of day

              // Skip classes before student's enrollment date
              if (studentCreatedAt && dayDate < studentCreatedAt) {
                return; // Don't include classes before student was added to system
              }

              dayData.studentsWithClass.forEach(classInfo => {
                if (
                  (classInfo.student.id === studentId || classInfo.student.id === parseInt(studentId)) &&
                  classInfo.status === 'unpaid' &&
                  dayDate <= today
                ) {
                  // Only count classes up to today
                  unpaidClasses.push({
                    date: dayData.date,
                    dateStr: dayData.date.toISOString().split('T')[0],
                    pricePerClass: classInfo.pricePerClass,
                    status: classInfo.status,
                    isClickedDate: dayData.date.toISOString().split('T')[0] === clickedDateStr,
                  });
                }
              });
            });

            debugLog('💰 Total unpaid classes found:', unpaidClasses.length);
            debugLog('📅 Unpaid dates:', unpaidClasses.map(c => c.dateStr).join(', '));

            if (unpaidClasses.length === 0) {
              console.log('ℹ️ No unpaid classes for:', student.name);
              showNotificationSimple('✅ No unpaid classes for ' + student.name, 'info');
              return;
            }

            // Check if the clicked date is actually unpaid
            const clickedDateIsUnpaid = unpaidClasses.some(c => c.isClickedDate);
            if (!clickedDateIsUnpaid) {
              console.warn('⚠️ Clicked date is not unpaid, but sending reminder for all unpaid classes');
            }
            ...
            // Sort by date to use clicked date (or most recent if clicked isn't unpaid)
            const primaryDate = clickedDateIsUnpaid
              ? unpaidClasses.find(c => c.isClickedDate)
              : unpaidClasses.sort((a, b) => new Date(b.dateStr) - new Date(a.dateStr))[0];

            debugLog('📅 Primary reminder date:', primaryDate.dateStr);
            debugLog('📧 Including all unpaid classes in email');

            // Send message to email iframe
            debugLog('🔍 Searching for email iframe...');
            const emailFrame = document.querySelector('iframe[src*="email-system-complete.html"]');
            console.log('📍 Email iframe found?', emailFrame ? 'YES' : 'NO');

            if (emailFrame) {
              console.log('📍 Iframe src:', emailFrame.src);
              console.log('📍 Has contentWindow?', emailFrame.contentWindow ? 'YES' : 'NO');
            } else {
              // Try to find ANY iframes
              const allIframes = document.querySelectorAll('iframe');
              debugLog('🔍 Total iframes on page:', allIframes.length);
              allIframes.forEach((iframe, idx) => {
                debugLog(`  Iframe ${idx}:`, iframe.src || iframe.id || 'no src/id');
              });
            }
            ...
            debugLog('📧 Email iframe found, sending postMessage...');

            // Create a promise that resolves when we get a response
            const sendPromise = new Promise((resolve, reject) => {
              const timeout = setTimeout(() => {
                console.error('⏱️ Email send timeout after 30 seconds');
              ...
              const handleMessage = event => {
                ...
                if (event.data.action === 'emailSent' || event.data.action === 'emailError') {
                  ...
                  if (event.data.action === 'emailSent') {
                    ...
                    console.error('❌ Email error:', event.data.error);
            ...
            emailFrame.contentWindow.postMessage(
              {
                ...
                classDate: primaryDate.dateStr,
                unpaidClasses: unpaidClasses.map(c => ({
                  date: c.dateStr,
                  pricePerClass: c.pricePerClass,
                  isClickedDate: c.isClickedDate,
                })),
                manual: true, // Flag to bypass "already sent" check
              },
              '*'
            );

            debugLog('📧 Sent manual reminder request to email system');
            ...
            // Add notification with email content if available
            if (window.NotificationCenter) {
              const emailSubject = response.emailContent?.subject || 'Payment Reminder';
              await window.NotificationCenter.add(
                window.NotificationCenter.NotificationType.EMAIL,
                `Email Sent: ${emailSubject}`,
                `Sent to ${student.name} (${student.email}) - ${unpaidClasses.length} unpaid class${unpaidClasses.length !== 1 ? 'es' : ''}`,
                {
                  studentName: student.name,
                  metadata: {
                    recipientName: student.name,
                    ...
                    emailBody: response.emailContent?.body,
            ...
            setTimeout(() => {
              ...
              buttonElement.style.background = 'rgba(34, 197, 94, 0.2)';
            ...
            showNotificationSimple('❌ Failed to send: ' + error.message, 'error');
          }
        };
        ...
        `;
            ...
            <div style="padding: 14px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; margin-bottom: 10px; transition: all 0.2s;">
                ...
                <div style="flex: 1;">
                  <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <button onclick="event.stopPropagation(); toggleAutoReminder('${student.id}', this);"
                      ...
                      ${window.PaymentReminderManager && window.PaymentReminderManager.isPaused(student.id) ? '▶' : '⏸'}
                    </button>
              ...
              ${isCanceled && skipInfo.note ? `<div style="margin-top: 8px; padding: 8px; background: rgba(148,163,184,0.1); border-left: 3px solid #94a3b8; border-radius: 4px; font-size: 11px; color: rgba(255,255,255,0.7);">📋 Reason: ${escapeHtml(skipInfo.note)}</div>` : ''}
            </div>
          `;
            });
          ...
          if (existingPanel) {
            ...
            console.log('📝 Updating existing sidebar panel');
            ...
            <div style="display: flex; gap: 10px;">
              ...
              ${
                ...
                <div style="font-size: 18px; font-weight: 900; color: #94a3b8;">🚫</div>
          ...
          `;
            ...
            if (contentDiv) {
              ...
              debugLog('✅ Sidebar panel updated');
            }
          ...
          console.log('🆕 Creating new sidebar panel');
          const panelHTML = `
      ...
      `;
        ...
        function cancelGroupClassFromSidebar(groupName, dateStr) {
          ...
          if (window.SkipClassManager && window.SkipClassManager.cancelClass) {
            window.SkipClassManager.cancelClass(groupName, dateStr, reason || 'Class canceled');
            ...
            showNotification(`✅ Class canceled for ${groupName} on ${dateDisplay}`, 'success');
          } else {
      ...
      `;
        ...
        async function quickAddPayment(studentId, dateStr, amount) {
          ...
          try {
            console.log('💳 Creating quick payment via addPayment()...');
            const result = await addPayment(paymentData);

            if (result) {
              showToast('✅ Payment added & receipt sent!', 'success');
              renderCalendar();
        ...
        async function applyFromCredit(studentId, dateStr, pricePerClass) {
          ...
          try {
            ...
            if (student.email) {
              verboseLog('[Credit] 📧 Sending credit applied email...');
              const emailResult = await sendCreditAppliedEmail(student, dateStr, pricePerClass, newBalance);

              if (emailResult.success) {
                showToast(
                  `✅ Applied ${formatCurrency(pricePerClass, '$')} from credit\nNew Balance: ${formatCurrency(newBalance, '$')}`,
                  'success'
                );
                showNotification(`📧 Email sent to ${student.email}`, 'success');
                verboseLog('[Credit] ✅ Email sent successfully');
              } else {
                showToast(
                  `✅ Applied ${formatCurrency(pricePerClass, '$')} from credit\nNew Balance: ${formatCurrency(newBalance, '$')}`,
                  'success'
                );
                showNotification(`⚠️ Credit applied but email failed: ${emailResult.error}`, 'warning');
                console.error('[Credit] ❌ Email failed:', emailResult.error);
              }
            } else {
              showToast(
            ...
            debugLog('🔍 Checking for panels to close after credit apply...');

            // Close the detail modal only (not the sidebar)
            const modal = document.getElementById('classDetailModal');
            if (modal) {
              console.log('❌ Closing classDetailModal');
              modal.remove();
            }
            const overlay = document.getElementById('calendarDetailOverlay');
            if (overlay) {
              console.log('❌ Closing calendarDetailOverlay');
              overlay.remove();
            }

            debugLog('🔄 Calling renderCalendar after credit apply...');
            // Refresh calendar to update dot colors
            renderCalendar();

            debugLog('📋 Checking for sidebar panel after credit apply...');
            // Update the day details sidebar immediately if it's open
            const dayPanel = document.getElementById('dayDetailsPanel');
            if (dayPanel) {
              debugLog('✅ Sidebar panel found, updating after credit apply...');
              ...
              debugLog('⚠️ No sidebar panel found after credit apply');
            }
          } catch (error) {
          ...
          if (window.AbsentManager) {
            window.AbsentManager.markAbsent(numericId, dateStr)
              ...
              .catch(err => {
                ...
                showToast('❌ Error marking absent');
              });
          } else {
            // Fallback to localStorage
        ...
        // ============================================================================
        // PAYMENT REMINDER MANAGER - Auto-send reminders for unpaid classes ☁️ CLOUD SYNC
        // ============================================================================
        /**
        ...
        window.PaymentReminderManager = (function () {
          ...
          async function loadPausedFromSupabase() {
            try {
              ...
              if (error) {
                console.error('[PaymentReminderManager] ❌ Error loading paused students:', error);
                return;
              }
              ...
              // debugLog('✅ [PaymentReminderManager] Loaded', Object.keys(pausedStudents).length, 'paused states from Supabase');
            } catch (error) {
              console.error('[PaymentReminderManager] ❌ Exception loading paused students:', error);
            }
          }
          ...
          async function savePausedToSupabase(studentId, paused) {
            try {
              ...
              if (error) {
                console.error('[PaymentReminderManager] ❌ Error saving paused state:', error);
                return false;
              }

              pausedStudents[studentId] = paused;
              debugLog('✅ [PaymentReminderManager] Saved paused state for student:', studentId, paused);
              return true;
            } catch (error) {
              console.error('[PaymentReminderManager] ❌ Exception saving paused state:', error);
              return false;
            }
          }
          ...
          async function togglePause(studentId) {
            ...
            if (success) {
              showNotification(
                newState ? '⏸️ Auto-reminders paused for this student' : '▶️ Auto-reminders resumed for this student',
                'success'
              );
            }
            return success;
          ...
          function getClassEndTime(groupName, dateStr) {
            try {
              ...
              console.log(
                `[PaymentReminderManager] 🕐 Class end time for ${groupName} on ${dateStr}: ${endTime.toLocaleString('en-US', { timeZone: 'America/Los_Angeles', hour: '2-digit', minute: '2-digit', hour12: true })}`
              );
          ...
          async function sendPaymentReminder(student, dateStr, amount, unpaidClassesList = []) {
            try {
              ...
              console.log(
                '📧 [PaymentReminderManager] Sent auto-reminder for:',
                student.name,
                'Date:',
                dateStr,
                'Amount:',
          ...
          async function checkAndSendReminders() {
            try {
              ...
              if (localStorage.getItem('automation_system_payment_reminder_paused') === 'true') {
                console.log('[PaymentReminderManager] ⏸️ PAUSED - Skipping reminder check');
                return;
              }
              ...
              debugLog('═══════════════════════════════════════════════════════');
              debugLog('[PaymentReminderManager] 🔍 STARTING DAILY REMINDER CHECK');
              debugLog('[PaymentReminderManager] 📅 LA Date:', todayStr);
              console.log(
                '[PaymentReminderManager] 🕐 LA Time:',
              ...
              if (lastCheckDate === todayStr) {
                debugLog('[PaymentReminderManager] ⏭️  Already checked today - skipping');
                debugLog('═══════════════════════════════════════════════════════');
                return;
              }
              ...
              if (!window.currentCalendarData) {
                debugLog('[PaymentReminderManager] ⏳ Calendar not initialized yet');

                // Try to render calendar now if function exists
                if (typeof renderCalendar === 'function') {
                  debugLog('[PaymentReminderManager] � Manually rendering calendar...');
                  renderCalendar();
                  debugLog('[PaymentReminderManager] ✅ Calendar rendered!');
                } else {
                  debugLog('[PaymentReminderManager] ❌ renderCalendar function not available');
                  debugLog('[PaymentReminderManager] 🔄 Will retry on next interval');
                  debugLog('═══════════════════════════════════════════════════════');
                  return;
                }
                ...
                if (!window.currentCalendarData) {
                  debugLog('[PaymentReminderManager] ❌ Calendar still not initialized after render attempt');
                  debugLog('[PaymentReminderManager] 🔄 Will retry on next interval');
                  debugLog('═══════════════════════════════════════════════════════');
                  return;
                }
              ...
              if (!calendarData || !calendarData.days) {
                console.warn('[PaymentReminderManager] ❌ No calendar data available');
                debugLog('═══════════════════════════════════════════════════════');
                return;
              }
              ...
              debugLog('[PaymentReminderManager] 📊 Checking', studentsArray.length, 'students for unpaid classes');
              ...
              for (const studentData of studentsArray) {
                ...
                if (student.status !== 'paused') {
                  debugLog(`  ⏭️  Student status is '${student.status}' (not paused) - skipping payment reminder`);
                  studentsPaused++;
                  continue;
                }
                ...
                if (isPaused(studentId)) {
                  debugLog('  ⏸️  Auto-reminders PAUSED for this student - skipping');
                  studentsPaused++;
                  continue;
                }
                ...
                if (unpaidClasses.length === 0) {
                  debugLog('  ✅ No unpaid classes (past or present) - skipping');
                  studentsNoUnpaid++;
                  continue;
                }

                debugLog(`  � Found ${unpaidClasses.length} unpaid class(es) - checking each...`);
                ...
                for (const unpaidClass of unpaidClasses) {
                  ...
                  console.log(
                    `\n  📅 Class Date: ${dateStr} | Status: ${unpaidClass.status} | Balance: $${unpaidClass.balance}`
                  );

                  // Skip if reminder already sent for this date
                  if (wasReminderSentToday(studentId, dateStr)) {
                    debugLog('    ✅ Reminder already sent for this date - skipping');
                    studentsAlreadySent++;
                    continue;
                  }
                  ...
                  if (!classEndTime) {
                    console.warn('    ❌ Could not determine class end time - skipping');
                    continue;
                  }
                  ...
                  if (nowLA < thirtyMinutesBeforeEnd) {
                    console.log(
                      `    ⏰ Too early for reminder | Current: ${nowLAStr} | Reminder at: ${reminderWindowStr} (30 min before ${classEndStr}) - skipping`
                    );
                  ...
                  debugLog(
                    `    ✅ REMINDER WINDOW ACTIVE | Current: ${nowLAStr} | Window started: ${reminderWindowStr} | Class ends: ${classEndStr}`
                  );
                  ...
                  console.log(`    📧 Sending payment reminder for ${unpaidClasses.length} unpaid class(es)`);
              ...
              console.log('\n═══════════════════════════════════════════════════════');
              console.log('[PaymentReminderManager] 📊 SUMMARY');
              ...
              if (remindersSent > 0) {
                debugLog('✅ [PaymentReminderManager] Successfully sent', remindersSent, 'auto-reminder(s)');
              } else {
                debugLog('✅ [PaymentReminderManager] No reminders needed today');
              }
              debugLog('═══════════════════════════════════════════════════════\n');
            } catch (error) {
              console.error('[PaymentReminderManager] ❌ ERROR in checkAndSendReminders:', error);
          ...
          async function initialize() {
            ...
            debugLog('[PaymentReminderManager] 🚀 Automation engine initialized');
            debugLog('[PaymentReminderManager] 📅 Using LA Timezone for ALL date/time calculations');
            ...
            window.addEventListener('message', async event => {
              ...
              if (event.data.action === 'autoReminderSent' && event.data.success) {
                ...
                debugLog('[PaymentReminderManager] ✅ Auto-reminder sent successfully!');
                ...
                if (window.NotificationCenter && emailContent) {
                  ...
                  debugLog('[PaymentReminderManager] ✅ Notification created with email content');
                }
              }
            });
            ...
            window.addEventListener('calendar:initialized', () => {
              debugLog('[PaymentReminderManager] 📊 Calendar initialized - scheduling first check in 5 seconds...');
              setTimeout(() => {
                debugLog('[PaymentReminderManager] 🔄 Running first automated reminder check...');
            ...
            if (window.currentCalendarData) {
              console.log(
                '[PaymentReminderManager] 📊 Calendar already initialized - scheduling check in 5 seconds...'
              );
              setTimeout(() => {
                debugLog('[PaymentReminderManager] 🔄 Running first automated reminder check...');
                checkAndSendReminders();
            ...
            debugLog('[PaymentReminderManager] ⏰ Scheduling hourly checks (every 60 minutes)');
            setInterval(
              () => {
                debugLog('[PaymentReminderManager] 🔄 Running hourly automated check...');
                checkAndSendReminders();
              },
              60 * 60 * 1000
            );
        ...
        window.ClassReminderManager = (function () {
          ...
          async function sendClassReminder(student, classDate, classTime, groupName, paymentStatus, creditBalance) {
            ...
            if (paymentStatus === 'paid') {
              paymentMessage =
                '<p style="color: #10b981; font-weight: 600;">✅ Your payment for this class is confirmed!</p>';
              ...
              paymentMessage = `<div class="info-box" style="background-color: #fef3c7; border-left-color: #f59e0b;">
                <p style="margin: 4px 0;"><strong>💳 Credit Available:</strong> Your credit balance of $${creditBalance} will be applied to this class.</p>
              ...
              paymentMessage = `<div class="info-box" style="background-color: #fef2f2; border-left-color: #ef4444;">
                <p style="margin: 4px 0;"><strong>⚠️ Payment Reminder:</strong> Please make your payment before class starts to avoid delays in notes being uploaded to Google Classroom.</p>
              </div>
              <div class="payment-frame">
                <p style="margin: 0 0 8px 0; font-size: 16px; color: #4b5563;">
                  <strong>Send Money with <span style="color: #764ba2;">Zelle®</span></strong>
                </p>
          ...
          async function checkAndSendReminders() {
            try {
              ...
              if (localStorage.getItem('automation_system_class_reminder_12h_paused') === 'true') {
                console.log('[ClassReminderManager] ⏸️ PAUSED - Skipping reminder check');
                return;
              }
              ...
              console.log('\n═══════════════════════════════════════════════════════');
              console.log('[ClassReminderManager] 🔍 STARTING CLASS REMINDER CHECK');
              console.log('[ClassReminderManager] 📅 LA Date:', todayStr);
              console.log(
                '[ClassReminderManager] 🕐 LA Time:',
              ...
              if (!window.currentCalendarData) {
                debugLog('[ClassReminderManager] ⏳ Calendar not initialized yet');
                console.log('═══════════════════════════════════════════════════════\n');
                return;
              }
              ...
              const calendarData = window.currentCalendarData;
              if (!calendarData || !calendarData.days) {
                console.warn('[ClassReminderManager] ❌ No calendar data available');
                console.log('═══════════════════════════════════════════════════════\n');
                return;
              }
                ...
                for (const classData of dayData.studentsWithClass) {
                  ...
                  if (hoursUntilClass >= 11.5 && hoursUntilClass <= 12.5) {
                    ...
                    if (wasReminderSent(student.id, dateStr)) {
                      debugLog(`[ClassReminderManager] ⏭️ Already sent reminder to ${student.name} for ${dateStr}`);
                      continue;
                    }
                    ...
                    console.log(`[ClassReminderManager] 📧 Sending class reminder to ${student.name}`);
                    console.log(
                    ...
                    if (sent) {
                      ...
                      remindersSent++;
                      console.log(`[ClassReminderManager] ✅ Reminder sent to ${student.name}`);
                    }
                  }
                }
              }
              ...
              console.error('[ClassReminderManager] ❌ Error checking reminders:', error);
            }
          }
          ...
          async function initialize() {
            loadSentReminders();
            console.log('[ClassReminderManager] 🚀 Class reminder system initialized');
            console.log('[ClassReminderManager] 📅 Will send reminders 12 hours before each class');
            ...
            window.addEventListener('message', async event => {
              if (event.data.action === 'classReminderSent' && event.data.success) {
                ...
                console.log('[ClassReminderManager] ✅ Class reminder sent successfully!');
              ...
              if (event.data.action === 'classReminderError') {
                console.error('[ClassReminderManager] ❌ Error sending class reminder:', event.data.error);
              }
            });

            // Listen for calendar initialization
            window.addEventListener('calendar:initialized', () => {
              debugLog('[ClassReminderManager] 📊 Calendar initialized - scheduling first check in 10 seconds...');
              setTimeout(() => {
                checkAndSendReminders();
              }, 10000);
            });
          ...
          async function checkAndSendReminders() {
            try {
              ...
              if (localStorage.getItem('automation_system_class_starting_soon_paused') === 'true') {
                console.log('[ClassStartingSoonManager] ⏸️ PAUSED - Skipping reminder check');
                return;
              }
              ...
              console.log('\n═══════════════════════════════════════════════════════');
              console.log('[ClassStartingSoonManager] 🔍 CHECKING 30-MIN REMINDERS');
              console.log('[ClassStartingSoonManager] 📅 LA Date:', todayStr);
              console.log(
                '[ClassStartingSoonManager] 🕐 LA Time:',
                nowLA.toLocaleString('en-US', {
                  timeZone: 'America/Los_Angeles',
                  hour: '2-digit',
                  minute: '2-digit',
              ...
              if (!window.currentCalendarData) {
                debugLog('[ClassStartingSoonManager] ⏳ Calendar not initialized yet');
                console.log('═══════════════════════════════════════════════════════\n');
                return;
              }
              ...
              const calendarData = window.currentCalendarData;
              if (!calendarData || !calendarData.days) {
                console.warn('[ClassStartingSoonManager] ❌ No calendar data available');
                console.log('═══════════════════════════════════════════════════════\n');
                return;
              }
              ...
              for (const dayData of calendarData.days) {
                ...
                for (const classData of dayData.studentsWithClass) {
                  ...
                  if (minutesUntilClass >= 25 && minutesUntilClass <= 35) {
                    if (wasReminderSent(student.id, dateStr)) {
                      debugLog(`[ClassStartingSoonManager] ⏭️ Already sent to ${student.name} for ${dateStr}`);
                      continue;
                    }
                    ...
                    console.log(`[ClassStartingSoonManager] 📧 Sending to ${student.name}`);
                    ...
                    if (sent) {
                      markReminderSent(student.id, dateStr);
                      remindersSent++;
                      console.log(`[ClassStartingSoonManager] ✅ Reminder sent to ${student.name}`);
                    }
                  }
                }
              }

              console.log('\n[ClassStartingSoonManager] 📊 SUMMARY');
              console.log('  Classes checked:', classesChecked);
              console.log('  Reminders sent:', remindersSent);
              console.log('═══════════════════════════════════════════════════════\n');
            } catch (error) {
              console.error('[ClassStartingSoonManager] ❌ Error:', error);
            }
          }

          async function initialize() {
            loadSentReminders();
            console.log('[ClassStartingSoonManager] 🚀 30-minute reminder system initialized');

            // Listen for email responses
            window.addEventListener('message', async event => {
              if (event.data.action === 'classStartingSoonSent' && event.data.success) {
                const { student, classDate, emailContent } = event.data;
                console.log('[ClassStartingSoonManager] ✅ Email sent to', student.name);
        ...
        window.addEmailPreviewToOldNotifications = async function () {
          verboseLog('[EmailPreview] 🔧 Reconstructing email previews for old notifications...');
          ...
          if (paymentNotifications.length === 0) {
            verboseLog('[EmailPreview] ✅ All notifications already have email previews!');
            return;
          }
          ...
          const iframe = document.querySelector('iframe[src*="email-system-complete.html"]');
          if (!iframe || !iframe.contentWindow) {
            console.error('[EmailPreview] ❌ Email system iframe not found');
          ...
          if (!template) {
            console.error('[EmailPreview] ❌ Could not get email template');
            return;
          }

          verboseLog('[EmailPreview] ✅ Got email template, reconstructing emails...');
          ...
          for (const notification of paymentNotifications) {
            ...
            try {
              ...
              if (error) {
                console.error(`[EmailPreview] ❌ Failed to update notification ${id}:`, error);
                failed++;
              } else {
                updated++;
              }
            } catch (err) {
              console.error(`[EmailPreview] ❌ Exception updating notification ${id}:`, err);
              failed++;
            }
          }

          console.log(`[EmailPreview] ✅ Updated ${updated} notifications with email previews!`);
          if (failed > 0) {
            console.warn(`[EmailPreview] ⚠️ Failed to update ${failed} notifications`);
          }
          verboseLog('[EmailPreview] 🎉 You can now click any payment reminder notification to see the email preview');
        ...
        window.NotificationCenter = (function () {
          ...
          async function loadFromSupabase() {
            ...
            if (!isAuthenticated || !currentUser) {
              debugLog('[NotificationCenter] ⏭️ Skipping load - not authenticated');
              return;
            }
            ...
            try {
              ...
              if (error) {
                ...
                if (
                  ...
                  debugLog('[NotificationCenter] ⏭️ Skipping - auth required');
                  return;
                }
              ...
              debugLog('[NotificationCenter] ✅ Loaded', notifications.length, 'notifications');
            } catch (error) {
              console.error('[NotificationCenter] ❌ Exception loading notifications:', error);
            }
          }
          ...
          async function saveToSupabase(notification) {
            // Skip if not authenticated
            if (!isAuthenticated || !currentUser) {
              debugLog('[NotificationCenter] ⏭️ Skipping save - not authenticated');
              return null;
            }
            ...
            try {
              ...
              debugLog('[NotificationCenter] 💾 Saving notification:', {
              ...
              debugLog('[NotificationCenter] 📦 Payload after truncation:', {
                title: payload.title,
              ...
              if (error) {
                ...
                if (
                  ...
                  debugLog('[NotificationCenter] ⏭️ Skipping save - auth required');
                  return null;
                }
                console.error('[NotificationCenter] ❌ Error saving notification:', error);
                return null;
              }
              ...
              if (error.message?.includes('Load failed') || error.message?.includes('NetworkError')) {
                debugLog('[NotificationCenter] ⏭️ Skipping save - network/auth error');
                return null;
              }
              console.error('[NotificationCenter] ❌ Exception saving notification:', error);
              return null;
            }
          }
          ...
          async function add(typeOrObject, title, description, options = {}) {
            ...
            if (saved) {
              ...
              debugLog('[NotificationCenter] 🔔 New notification:', notification.title);
            }
          ...
          async function markAsRead(notificationId) {
            try {
              ...
              console.error('[NotificationCenter] ❌ Exception marking as read:', error);
              return false;
            }
          }
          ...
          async function markAllAsRead() {
            try {
              ...
              console.error('[NotificationCenter] ❌ Exception marking all as read:', error);
              return false;
            }
          }
          ...
          // Initialize
          async function initialize() {
            debugLog('[NotificationCenter] 🔄 Initializing...');
            await loadFromSupabase();
            debugLog('[NotificationCenter] 🔔 Notification Center initialized');
            debugLog('[NotificationCenter] 📊 Loaded', notifications.length, 'notifications');
            debugLog('[NotificationCenter] 📬 Unread count:', unreadCount);

            // Add a test notification on first load (only if no notifications exist)
            if (notifications.length === 0) {
              debugLog('[NotificationCenter] 🧪 Adding test notification (first load)...');
              await add(
        ...
        document.addEventListener('DOMContentLoaded', async function () {
          console.log('✅ ARNOMA v2.7.5 - Original Buttons Restored ✅');
          ...
          // ⚡ CRITICAL: Only load CORE data immediately
          await initializeCoreData();
          console.log('✅ Core data loaded');

          // ⚡ Defer non-critical systems using requestIdleCallback
        ...
        // ⚡ FIX #1: PHASE 2 - Defer everything else to idle time
            ...
            scheduleWork(() => {
              try {
                ...
                if (result && typeof result.then === 'function') {
                  result.catch(error => console.error('⚠️ Deferred task failed:', error));
                }
              } catch (error) {
                console.error('⚠️ Deferred task failed:', error);
              }
            });
          };

          // ⚡ FIX #1: Delay heavy systems until after UI is visible
        ...
        console.log('⚡ ARNOMA Performance Mode - Lazy loading enabled');
