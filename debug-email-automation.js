...
if (!iframe) {
  ...
  groups.forEach(group => {
    ...
    sessions.forEach(session => {
      ...
      const currentDay = now.toLocaleString('en-US', { timeZone: 'America/Los_Angeles', weekday: 'short' });
          ...
          activeAutomations.forEach(auto => {
            ...
            if (diff <= 2) {
              ...
              if (!auto.selectedGroups || !auto.selectedGroups.includes(group.name)) {
                console.warn(`      ⚠️  But group "${group.name}" NOT selected in automation "${auto.name}"`);
              } else {
                console.log(`      ✅ Group selected in automation "${auto.name}"`);
              }
            } else if (minutesUntil > 0 && minutesUntil < 120) {
              console.log(`      ⏰ Coming up: ${minutesUntil} min (trigger: ${triggerTime} min before)`);
            }
          });
        }
      });
