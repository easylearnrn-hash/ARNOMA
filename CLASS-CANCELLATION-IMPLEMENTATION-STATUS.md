...
- ✅ `forwardPaymentsForCanceledClass(groupName, dateStr)` - Auto-forward payments (NEW)
- ✅ `findNextActiveClassDate(groupName, fromDateStr)` - Find next non-canceled class (NEW)
...
**Current:** Calls `SkipClassManager.skipClass()` (student skip)
**Needed:** Add option to cancel entire class vs skip individual student
...
6. ❌ Payment expectation logic
7. ❌ UI buttons to cancel class

**LOW PRIORITY (Nice to have):**
8. ❌ Reverse cancellation (undo)
9. ❌ Mobile UI consistency
...
- Backward compatible: existing records will work without migration (defaults applied)
