...
(function () {
  ...
  if (window.currentCalendarData && window.currentCalendarData.students) {
    for (const studentData of window.currentCalendarData.students) {
      ...
      if (
        student.name.toLowerCase().includes('mariam') &&
        student.name.toLowerCase().includes('gevorgyan')
      ) {
        mariam = student;
  ...
  if (!mariam) {
    ...
    // Sort by date
    const sortedAttendance = [...mariamData.attendance].sort((a, b) =>
    ...
    sortedAttendance.forEach(a => {
      ...
      if (paymentOnDate) {
        console.log(
          `      ✅ PAYMENT FOUND: $${(paymentOnDate.amount || 0).toFixed(2)} on ${new Date(paymentOnDate.emailDate || paymentOnDate.date).toISOString().split('T')[0]}`
        );
      } else {
    ...
    const unpaidClasses = mariamData.attendance.filter(
      a => a.status === 'unpaid' && a.date <= today
    );
    ...
    if (unpaidClasses.length > 0) {
      ...
      console.log('   ' + '─'.repeat(80));
      console.log(
        '   The system detected ' + unpaidClasses.length + ' unpaid class(es) and sent a reminder.'
      );
      ...
      console.log(
        '   If Mariam received an email, it may have been sent before payment was recorded.'
      );
