...
console.log(`   Day: ${nowLA.toLocaleString('en-US', { timeZone: 'America/Los_Angeles', weekday: 'long' })}\n`);
...
if (window.groupsData) {
  ...
  const currentDay = nowLA.toLocaleString('en-US', { timeZone: 'America/Los_Angeles', weekday: 'short' });
  
  window.groupsData.forEach(group => {
    if (group.schedule && group.schedule.includes(currentDay)) {
      // Parse time from schedule
      const timeMatch = group.schedule.match(new RegExp(currentDay + '\\s+(\\d+:\\d+\\s+(?:AM|PM))', 'i'));
      if (timeMatch) {
  ...
  if (upcomingClasses.length > 0) {
    ...
    upcomingClasses.forEach(c => {
      ...
      console.log(`         Group ${c.group}: ${c.time} (${c.minutesUntil} min) ${inWindow ? '🔥 IN TRIGGER WINDOW!' : ''}`);
    });
  } else {
